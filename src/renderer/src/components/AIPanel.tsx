...
/** AI 助手抽屉:右侧滑出的浮层,收起时不占写作区;生成时由 store 自动弹出 */
export default function AIPanel() {
  ...
  return (
    <aside className="drawer" aria-label="AI 助手">
      {/* 抽屉头部:标题 + 模型下拉 + 关闭 */}
      <div className="flex shrink-0 items-center gap-1.5 px-4 pb-2 pt-4">
        ...
        <div className="relative ml-1 min-w-0 flex-1">
          <button
            className="flex min-w-0 items-center gap-1 text-xs font-medium t3 hover:text-[var(--text)]"
          ...
          {profileMenu && (
            <>
              ...
              <div className="panel absolute left-0 top-6 z-20 w-56 p-1" style={{ boxShadow: 'var(--shadow-pop)' }}>
        ...
        <button className="btn-ghost !h-7 !w-7 shrink-0 !px-0" title="收起 (Ctrl+I)" onClick={() => setAiOpen(false)}>
          <X size={16} />
        </button>
      </div>
      ...
      {needsApiKey && (
        <div
          className="mx-4 mb-3 shrink-0 rounded-xl p-3 text-xs leading-relaxed"
          style={{
      ...
      {/* 生成 / 灵感 切换 */}
      <div className="mx-4 mb-3 flex w-[calc(100%-2rem)] shrink-0">
        <div className="seg w-full">
          <button className={`seg-item flex-1 ${tab === 'gen' ? 'active' : ''}`} onClick={() => setTab('gen')}>
            <Wand2 size={14} />
            生成
          </button>
          <button className={`seg-item flex-1 ${tab === 'chat' ? 'active' : ''}`} onClick={() => setTab('chat')}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pb-4">
        ...
        {tab === 'gen' && (
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto pr-1">
            ...
            {hasSelection && (
              <div
                className="mb-4 flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-xs"
            ...
            {/* 续写创作 —— 「生成整章」是面板内唯一主色按钮 */}
            <section>
              <textarea
                className="field-input min-h-16 resize-y !text-[13px]"
            ...
            {/* 底部弱化提示:生成注入规模(一键去设定中心) */}
            <button
              className="mt-5 flex w-full shrink-0 items-center gap-2 rounded-xl px-1 py-1 text-left text-[11px] t3 transition hover:text-[var(--text)]"
            ...
            {aiRunning && (
              <div
                className="mt-4 flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-sm"
                style={{ background: 'var(--accent-soft)', color: 'var(--accent-ink)' }}
              >
            ...
            {aiError && (
              <div
                className="mt-4 shrink-0 rounded-xl px-3 py-2 text-xs leading-relaxed"
                style={{
            ...
            {aiOutput && (
              <div className="mt-4 flex min-h-0 flex-1 flex-col">
                ...
                <div className="inset serif min-h-24 flex-1 whitespace-pre-wrap p-3 text-sm leading-relaxed">
                  {aiOutput}
                </div>
                {/* 输出落地:紧跟结果,一眼可用 */}
                {ACTIONS.length > 0 && !aiRunning && (
        ...
        {tab === 'chat' && (
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            {/* 输入区 */}
            <div className="mt-2 shrink-0" style={{ borderTop: '1px solid var(--border)', paddingTop: 10 }}>
              <textarea
...
/** 抽屉标题前的发光小标:主色渐变芯片 */
function SparklesIcon(): React.ReactNode {
  return (
    <span
      className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg"
      style={{
        background: 'linear-gradient(140deg, color-mix(in srgb, var(--accent) 88%, #fff 12%), var(--accent) 60%, var(--accent-hover))',
        color: '#ffffff',
        boxShadow: 'var(--inset-light), 0 3px 8px -3px color-mix(in srgb, var(--accent) 55%, transparent)'
      }}
    >
      <Wand2 size={13} />
    </span>
